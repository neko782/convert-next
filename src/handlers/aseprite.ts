...
function decodeAseprite(bytes: Uint8Array): ParsedAseprite {
  if (bytes.length < 128)
    throw new RangeError("File is too small to be a valid .aseprite file.");
  ...
  if (width === 0 || height === 0)
    throw new RangeError(
      `Invalid image dimensions in Aseprite file: ${width}x${height}`,
    );
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex++) {
    ...
    if (frameMagic !== ASEPRITE_FRAME_MAGIC)
      throw new BadMagicError(
        `Invalid frame magic in Aseprite file: ${frameMagic}`,
      );
    ...
    for (let i = 0; i < chunkCount; i++) {
      ...
      if (chunkSize < 6 || chunkEnd > bytes.length || chunkEnd > frameEnd)
        throw new RangeError(
          `Invalid chunk size in Aseprite file: ${chunkSize}`,
        );
      ...
      if (chunkType === CHUNK_TYPE_LAYER) {
        ...
        for (
          let packet = 0;
          packet < packetCount && entryOffset + 2 <= chunkEnd;
          packet++
        ) {
        ...
        if (celType === 1) {
          ...
          const bytesPerPixel =
            colorDepth === 32 ? 4 : colorDepth === 16 ? 2 : 1;
          ...
          if (celType === 0) {
            ...
            if (rawEnd > chunkEnd)
              throw new RangeError(`Invalid raw cel data length: ${rawEnd}`);
            ...
            if (celBytes.length !== expectedByteLength)
              throw new RangeError(
                `Invalid decompressed cel size: ${celBytes.length}`,
              );
          }
    ...
    if (chunkOffset > frameEnd)
      throw new RangeError(
        `Frame chunk data exceeds frame bounds: ${chunkOffset} > ${frameEnd}`,
      );
  ...
  const layerEntries = Array.from(targetFrame.values()).sort(
  ...
  for (const cel of layerEntries) {
    ...
    for (let py = 0; py < cel.height; py++) {
      ...
      for (let px = 0; px < cel.width; px++) {
        ...
        const srcAlpha =
          (cel.pixels[srcIndex + 3] / 255) * celOpacity * layerOpacity;
        ...
        const blendedR = clamp01(
          blend(blendMode, srcR, dstR, srcRgb, dstRgb, "r"),
        );
        const blendedG = clamp01(
          blend(blendMode, srcG, dstG, srcRgb, dstRgb, "g"),
        );
        const blendedB = clamp01(
          blend(blendMode, srcB, dstB, srcRgb, dstRgb, "b"),
        );
