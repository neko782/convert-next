...
class VexFlowHandler implements FormatHandler {
  ...
  private musicXMLToMidiEvents(xmlString: string): any[] {
    ...
    if (!scorePartwise && !scoreTimewise) {
      ...
      throw new Error(
        "Invalid MusicXML: missing score-partwise or score-timewise element",
      );
    }
    ...
    const divisions = divisionsEl
      ? parseInt(divisionsEl.textContent || "1")
      : 1;
    const ticksPerDivision = ticksPerBeat / divisions;

    console.log(
      `MusicXML: divisions=${divisions}, ticksPerDivision=${ticksPerDivision}`,
    );
      ...
      measures.forEach((measure, measureIndex) => {
        ...
        notes.forEach((note) => {
          ...
          const duration = durationEl
            ? parseInt(durationEl.textContent || "0")
            : 0;
          ...
          if (!isRest) {
            ...
            if (pitchEl) {
              ...
              if (stepEl && octaveEl) {
                ...
                const alter = alterEl
                  ? parseInt(alterEl.textContent || "0")
                  : 0;
    ...
    const usedTracks = new Set(
      events.filter((e) => e.track >= 0).map((e) => e.track),
    );
    events.unshift(
      {
        track: -1,
  ...
  async doConvert(
    ...
    for (const inputFile of inputFiles) {
      try {
        ...
        if (
          inputFormat.internal === "mxl" ||
          inputFile.name.toLowerCase().endsWith(".mxl")
        ) {
