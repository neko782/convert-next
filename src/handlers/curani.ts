...
class curaniHandler implements FormatHandler {
  ...
  async doConvert(
    ...
    const ani_separator_bytes = new Uint8Array([
      0x69, 0x63, 0x6f, 0x6e, 0xbe, 0x10, 0x00, 0x00,
    ]);
      ...
      if (inputFormat.internal === "ani") {
        ...
        if (outputFormat.internal === "cur") {
          ...
          // The code could find another header, use the distance.
            ...
            new_file_bytes = new_file_bytes.subarray(
              ico_start,
              ico_start + ico_distance,
            );
          }
        ...
        if (outputFormat.internal === "ani") {
          const ani_header = new Uint8Array([
            0x52, 0x49, 0x46, 0x46, 0xc8, 0x21, 0x00, 0x00, 0x41, 0x43, 0x4f,
            0x4e, 0x61, 0x6e, 0x69, 0x68, 0x24, 0x00, 0x00, 0x00, 0x24, 0x00,
            0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
            0x00, 0x4c, 0x49, 0x00, 0x54, 0x90, 0x21, 0x00, 0x00, 0x66, 0x72,
            0x61, 0x6d, 0x69, 0x63, 0x6f, 0x6e, 0xbe, 0x10, 0x00, 0x00,
          ]);

          let mergedArray = new Uint8Array(
            ani_header.length + new_file_bytes.length,
          );
        ...
        // Convert a .cur into a .ico by ADDING hotspot and changing format header
          ...
          const images_present = read_lendian(
            new_file_bytes[4],
            new_file_bytes[5],
          );
          let counter = 0;
        ...
        inputFormat.internal === "apng" &&
        outputFormat.internal === "ani"
      ) {
        // To be added!
        throw new TypeError(
