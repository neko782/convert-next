...
class comicsHandler implements FormatHandler {
  ...
  async doConvert(
    ...
    // Single-gif catching
    if (
      inputFormat.internal === "gif" &&
      archives_list.includes(outputFormat.internal) &&
      inputFiles.length === 1
    ) {
      throw new TypeError(
        "User probably intends for an archive of video/gif frames; abort.",
      );
    }
    ...
    if (
      ...
      for (const file of inputFiles) {
        if (outputFormat.internal === "cbz") {
          zip.file(
            "Page " + String(iterations) + "." + inputFormat.extension,
            file.bytes,
          );
        } else {
        ...
        for (const [filename, zipEntry] of Object.entries(zip.files)) {
          if (!zipEntry.dir) {
            if (inputFormat.internal === "cbz" && filename.endsWith(".xml")) {
              ...
              filename.endsWith("." + outputFormat.extension) === false
            ) {
              throw new TypeError(
                "Archive contains multiple file types; abort.",
              );
            } else {
    ...
    // Pack a cbt with code from tar.ts
      ...
      image_list.includes(inputFormat.internal) &&
      outputFormat.internal === "cbt"
    ) {
      const bytes = createTar(
        inputFiles.map((file) => ({
          name:
            "Page " + inputFiles.indexOf(file) + "." + inputFormat.extension,
          data: file.bytes,
        })),
        {},
      );
      ...
      for (const inputFile of inputFiles) {
        ...
        for (const file of files) {
          if (inputFormat.internal === "cbt" && file.name.endsWith(".xml")) {
            ...
            file.name.endsWith("." + outputFormat.extension) === false
          ) {
    ...
    // Renaming interchangeable formats. Note that any valid "comic book" archive can be guaranteed as a valid standard archive, but not every valid archive can be a valid comic book archive. Thus, we only allow renaming from comic book to non-comic book formats.
      ...
      (inputFormat.internal === "cbz" && outputFormat.internal === "zip") ||
      (inputFormat.internal === "cbt" && outputFormat.internal === "tar")
    ) {
      for (const file of inputFiles) {
        outputFiles.push({
          bytes: file.bytes,
