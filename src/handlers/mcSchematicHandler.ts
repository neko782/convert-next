...
class mcSchematicHandler implements FormatHandler {
  ...
  async doConvert(
    ...
    for (const file of inputFiles) {
      ...
      if (inputFormat.internal === "litematic") {
        ...
        inputFormat.internal === "schematic" ||
        inputFormat.internal === "schem"
      ) {
      ...
      for (let y = 0; y < height; y++) {
        for (let z = 0; z < length; z++) {
          for (let x = 0; x < width; x++) {
            ...
            if (
              bitOffset + BigInt(bitsPerBlock) > 64n &&
              longIndex + 1 < longs.length
            ) {
  ...
  private schemToLitematic(data: any): NBT.NBTData {
    ...
    if (
      ...
      throw new Error(
        "Invalid Schematic file: Missing required size or block data tags.",
      );
    }
    ...
    for (let i = 0; i < paletteSize; i++) {
      ...
      if (bracketIndex > -1) {
        ...
        const propsString = rawName.substring(
          bracketIndex + 1,
          rawName.length - 1,
        );
        const props: any = {};
    ...
    const result = new NBT.NBTData(
      {
        MinecraftDataVersion: new NBT.Int32(
          root.DataVersion?.valueOf() || 2566,
        ),
