    ...
    if (
      ...
      for (const file of inputFiles) {
        ...
        for (let i = 0; i < numEntries; i++) {
          ...
          if (data_size === "0") {
            throw new Error(
              "Error, file has no data. Can't meaningfully convert.",
            );
          }
      ...
      if (inputFormat.internal === "zip") {
        for (const file of inputFiles) {
          ...
          for (const [filename, zipEntry] of Object.entries(zip.files)) {
            if (!zipEntry.dir) {
              if (filename.endsWith(".json") === false) {
                throw new Error(
                  "Archive contains more than just .json files; abort.",
                );
              } else {
          ...
          if (!done_something_flag) {
            throw new Error(
              "No applicable files to unzip found in " + file.name,
            );
          }
        }
      } else {
      ...
      for (const key in working_files) {
        ...
        for (let i = 0; i < working_files[key].length; i++) {
          ...
          working_bytes.push(
            ...write_lendian_4(working_files[key][i].bytes.length),
          );
        }
