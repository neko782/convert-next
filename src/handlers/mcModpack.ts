...
class mcModpackHandler implements FormatHandler {
  ...
  async doConvert(
    ...
    for (const file of inputFiles) {
      ...
      if (
        inputFormat.internal === "mrpack" &&
        outputFormat.internal === "zip"
      ) {
        const indexFile = zip.file("modrinth.index.json");
        if (!indexFile)
          throw new Error(
            "Invalid Modrinth modpack: missing modrinth.index.json",
          );
          ...
          if (downloadUrl) {
            try {
              ...
              if (res.ok) {
                ...
                outZip.file(
                  `overrides/${modFile.path}`,
                  new Uint8Array(buffer),
                );
              } else {
                console.warn(
                  `Failed to download ${downloadUrl} (status ${res.status})`,
                );
              }
            } catch (e) {
        ...
        const modLoaders = [];
        for (const [loader, version] of Object.entries(
          index.dependencies || {},
        )) {
          if (loader !== "minecraft") {
        ...
        if (!manifestFile)
          throw new Error(
            "Invalid CurseForge modpack: missing manifest.json inside zip.",
          );
        ...
        if (manifest.manifestType !== "minecraftModpack") {
          throw new Error(
            `Unsupported manifest type: ${manifest.manifestType}`,
          );
        }
        ...
        zip.forEach((relativePath, zipEntry) => {
          if (!zipEntry.dir && relativePath.startsWith(overridesStr + "/")) {
            overridePromises.push(
              zipEntry.async("uint8array").then((data) => {
                const modrinthPath = relativePath.replace(
                  overridesStr + "/",
                  "overrides/",
                );
        ...
        for (const modFile of manifest.files || []) {
          try {
            ...
            console.error(
              `Error downloading CF mod ${modFile.projectID}:${modFile.fileID}`,
              e,
            );
          }
        }
