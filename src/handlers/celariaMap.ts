...
class celariaMapHandler implements FormatHandler {
  ...
  async doConvert(
    ...
    return inputFiles.map((file) => {
      ...
      if (inputFormat.internal === "ecmap") {
        const editableCelariaMap = EditableCelariaMap.parse(
      ...
      if (inputFormat.internal === "json") {
        const typedParsedObject =
          celariaMapHandler.determinePlainOldSerializedFormat(
            JSON.parse(new TextDecoder().decode(file.bytes)),
          );
        if (!typedParsedObject)
          throw new Error("Can't handle unknown parsed object.");
        function putInstances(
          ...
          map.instances = plainOldInstances.map((plainOldInstance) => {
            switch (plainOldInstance.instanceType) {
              ...
              case "Dummy":
                const tutorialHologram = new TutorialHologram(
                  plainOldInstance.type,
                );
        ...
        if (outputFormat.internal === "ecmap") {
          ...
          editableCelariaMap.sunRotationHorizontal =
            typedParsedObject.sunRotationHorizontal;
          editableCelariaMap.sunRotationVertical =
            typedParsedObject.sunRotationVertical;
          editableCelariaMap.instances.forEach((instance) => {
            if (
              instance.instanceId === 0 &&
              instance.type === Block.types.checkpoint
            )
              editableCelariaMap.checkpointOrder.add(instance);
          });
          const goal = editableCelariaMap.instances.find(
            (instance) =>
              instance.instanceId === 0 && instance.type === Block.types.goal,
          ) as Block | undefined;
          if (goal) editableCelariaMap.checkpointOrder.add(goal);
          return {
            name: `${baseName}.${outputFormat.extension}`,
            bytes: Uint8Array.from(
              editableCelariaMap.serialize(suitableVersion),
            ),
          };
        }
      }
      if (
        inputFormat.internal === "cmap" &&
        outputFormat.internal === "ecmap"
      ) {
        ...
        editableCelariaMap.sunRotationHorizontal =
          celariaMap.sunRotationHorizontal;
  ...
  static determinePlainOldSerializedFormat(
    ...
    if (plainOldObject.mapType === "Celaria map")
  ...
  static determineSuitableVersion(
    format: "ecmap" | "cmap",
    instances: Instance[],
  ): number {
    ...
    Object.values(ModelBuilder.directions).forEach((face) => {
      ...
      for (let i = 0; i < face.geometry.length / 3; i++) {
        ...
        [
          ...
          // perform rotation
          result[0] =
            scaledVertex[0] * Math.cos(radians) -
            scaledVertex[1] * Math.sin(radians);
          result[1] =
            scaledVertex[0] * Math.sin(radians) +
            scaledVertex[1] * Math.cos(radians);
...
// #region Types
type AllInstanceTypes =
  Block | PlayerSpawnPoint | Sphere | TutorialHologram | Barrier;
