...
export type ConversionOption =
  ConversionOptionsMap extends Map<infer K, infer V> ? [K, V] : never;
  ...
  for (const deadEnd of deadEndAttempts) {
    ...
    if (isDeadEnd) {
      const deadEndString = deadEnd
        ...
        .join(" → ");
      console.warn(
        `Skipping ${pathString} due to dead end near ${deadEndString}.`,
      );
      return null;
    }
  }
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    try {
      ...
      if (!handler.ready) {
        ...
        await handler.init();
        if (!handler.ready)
          throw `Handler "${handler.name}" not ready after init.`;
        if (handler.supportedFormats) {
      ...
      ctx.log(
        `Converting ${path[i].format.format} → ${path[i + 1].format.format}`,
      );
