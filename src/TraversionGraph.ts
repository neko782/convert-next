...
export class TraversionGraph {
  ...
  private categoryAdaptiveCosts: CategoryAdaptiveCost[] = [
    ...
    { categories: ["audio", "document", "image"], cost: 15 }, // We don't need a screenshot of an audio embed
  ...
  public removeCategoryChangeCost(
    from: string,
    to: string,
    handler?: string,
  ): boolean {
  ...
  public updateCategoryChangeCost(
    ...
    const costEntry = this.categoryChangeCosts.find(
      (c) =>
        c.from === from && c.to === to && c.handler === handler?.toLowerCase(),
    );
    ...
    supportedFormatCache.forEach((formats, handler) => {
      ...
      formats.forEach((format) => {
        ...
        let index = this.nodes.findIndex(
          (node) => node.identifier === formatIdentifier,
        );
        if (index === -1) {
    ...
    handlers.forEach((handler, hIndex) => {
      ...
      this.nodes.forEach((fromNode, fromIndex) => {
        for (const to of toEntries) {
          ...
          const alreadyExists = fromNode.edges.some(
            (eIdx) =>
              this.edges[eIdx].to.index === to.index &&
              this.edges[eIdx].handler === handler.name,
          );
  ...
  private costFunction(
    ...
    const handlerPairs = new Map<string, string>(
    ...
    cost +=
      FORMAT_PRIORITY_COST *
      (handlerObj?.supportedFormats?.findIndex(
        (f) => f.mime === to.format.mime,
      ) ?? 0);
  ...
  public async *searchPath(
    ...
    let fromIndex = this.nodes.findIndex(
      (node) => node.identifier === fromIdentifier,
    );
    let toIndex = this.nodes.findIndex(
      (node) => node.identifier === toIdentifier,
    );
    ...
    while (queue.size() > 0) {
      ...
      if (current.index === toIndex) {
        ...
        if (
          simpleMode ||
          !to.handler ||
          to.handler.name === foundPathLast?.handler.name
        ) {
