...
function applyMode(value: ModeEnum) {
  if (value === ModeEnum.Simple)
    document.documentElement.style.setProperty("--primary", "#1C77FF");
  if (value === ModeEnum.Advanced)
    document.documentElement.style.setProperty("--primary", "#FF6F1C");
}
...
export function toggleMode() {
  Mode.value =
    Mode.value === ModeEnum.Advanced ? ModeEnum.Simple : ModeEnum.Advanced;
}
