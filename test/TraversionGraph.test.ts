...
test("should find path from image to archive via anyinput\n", async () => {
  ...
  expect(extractedPaths[0][extractedPaths[0].length - 1].format.format).toBe(
    "zip",
  );
  expect(extractedPaths[0][extractedPaths[0].length - 1].handler.name).toBe(
    "archiver",
  );
});
